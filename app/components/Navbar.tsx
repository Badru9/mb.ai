...
export default function NavbarComponent() {
  ...
  return (
    <Surface className="flex justify-between py-4 px-5 sm:py-5 sm:px-10 fixed top-0 left-0 right-0 z-50">
      ...
      {isOpen && (
        <nav className="absolute top-full left-0 right-0 bg-background border-b border-border flex flex-col p-4 gap-2 md:hidden">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="no-underline py-2 px-4 hover:bg-surface rounded-md transition-colors"
              onPress={closeMenu}
            >
              {link.label}
            </Link>
